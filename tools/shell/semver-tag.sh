...
semverTag() {
  ...
  if [ -z "$CHANGED_FILES" ]; then
    ...
    CHANGED_FILES_CONTAIN_MAJOR_CHANGES=$(printf "%s" "$CHANGED_FILES" | grep -o "angular\.json\|nx\.json\|apps\/api\/.*\|apps\/client\/.*\|apps\/portfolio\/.*\|apps\/documentation\/.*\|libs\/backend-core\/.*\|libs\/client-core\/.*")
    local CHANGED_FILES_CONTAIN_MINOR_CHANGES
    CHANGED_FILES_CONTAIN_MINOR_CHANGES=$(printf "%s" "$CHANGED_FILES" | grep -v "angular\.json\|nx\.json\|apps\/api\/.*\|apps\/client\/.*\|apps\/portfolio\/.*\|apps\/documentation\/.*\|libs\/backend-core\/.*\|libs\/client-core\/.*" | grep -o "apps\/.*\|libs\/.*")
    local CHANGED_FILES_CONTAIN_PATCH_CHANGES
    CHANGED_FILES_CONTAIN_PATCH_CHANGES=$(printf "%s" "$CHANGED_FILES" | grep -v "angular\.json\|nx\.json\|apps\/api\/.*\|apps\/client\/.*\|apps\/portfolio\/.*\|apps\/documentation\/.*\|libs\/backend-core\/.*\|libs\/client-core\/.*\|apps\/.*\|libs\/.*" | grep -o ".*")
